...
export type VideoQuality = 'best' | '2160p' | '1440p' | '1080p' | '720p' | '480p' | '360p' | '240p' | '144p' | 'highest' | 'lowest';
...
export type AudioFormat = 'aac' | 'flac' | 'mp3' | 'm4a' | 'opus' | 'vorbis' | 'wav' | 'alac';
...
export type FormatArgs<F extends string = string> = F extends FormatKeyWord ? {
    filter: F;
...
export interface DownloadProcess extends ChildProcess {
    ...
    on<K extends keyof DownloadProcessEvents>(event: K, listener: (...args: DownloadProcessEvents[K]) => void): this;
    /** Adds a one-time listener for the specified event. */
    once<K extends keyof DownloadProcessEvents>(event: K, listener: (...args: DownloadProcessEvents[K]) => void): this;
    /** Removes a listener for the specified event. */
    off<K extends keyof DownloadProcessEvents>(event: K, listener: (...args: DownloadProcessEvents[K]) => void): this;
    /** Removes a listener for the specified event. */
    removeListener<K extends keyof DownloadProcessEvents>(event: K, listener: (...args: DownloadProcessEvents[K]) => void): this;
    /** Adds a listener to the beginning of the listeners array. */
    prependListener<K extends keyof DownloadProcessEvents>(event: K, listener: (...args: DownloadProcessEvents[K]) => void): this;
    /** Adds a one-time listener to the beginning of the listeners array. */
    prependOnceListener<K extends keyof DownloadProcessEvents>(event: K, listener: (...args: DownloadProcessEvents[K]) => void): this;
    /** Emits an event with the specified arguments. */
    emit<K extends keyof DownloadProcessEvents>(event: K, ...args: DownloadProcessEvents[K]): boolean;
    /** Returns the number of listeners for the specified event. */
    listenerCount(event: keyof DownloadProcessEvents): number;
    /** Returns a copy of the array of listeners for the specified event. */
    listeners(event: keyof DownloadProcessEvents): ((...args: unknown[]) => void)[];
